...
interface DayData { sessions: DaySession[]; dominant: AttendanceRecordStatus }
...
function buildDayMap(sections: StudentClassSection[]): Record<string, DayData> {
  ...
  const priority: AttendanceRecordStatus[] = ['ABSENT','LATE','PRESENT','NONE'];
  for (const sec of sections) {
    for (const sess of sec.sessions) {
      ...
      map[key].sessions.push({
        ...
        status, session_no: sess.session_no,
...
const S: Record<AttendanceRecordStatus, {
  label: string; dot: string; badge: string; icon: React.ReactNode;
}> = {
  PRESENT: { label:'Có mặt',   dot:'bg-emerald-500', badge:'bg-emerald-100 text-emerald-700 border border-emerald-200', icon:<CheckCircle2 className="h-3.5 w-3.5"/> },
  LATE:    { label:'Đi muộn',  dot:'bg-amber-400',   badge:'bg-amber-100 text-amber-700 border border-amber-200',     icon:<Timer       className="h-3.5 w-3.5"/> },
  ABSENT:  { label:'Vắng mặt', dot:'bg-red-500',     badge:'bg-red-100 text-red-700 border border-red-200',           icon:<XCircle     className="h-3.5 w-3.5"/> },
  NONE:    { label:'Chưa ghi', dot:'bg-slate-300',   badge:'bg-slate-100 text-slate-500 border border-slate-200',     icon:<Minus       className="h-3.5 w-3.5"/> },
};

function Badge({ status }: { status: AttendanceRecordStatus }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${S[status].badge}`}>
      {S[status].icon}{S[status].label}
    </span>
  );
}
...
function HoverPopup({ dateKey, data, rect, onClose, onKeepOpen, onDetail }: PopupProps) {
  const POPUP_W = 220;
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-border px-4 py-3">
        ...
        <p className="text-[11px] text-muted-foreground">{data.sessions.length} buổi học</p>
      </div>
      ...
      <div className="divide-y divide-border/50">
        {data.sessions.slice(0, 2).map((s, i) => (
          <div key={i} className="flex items-center justify-between gap-2 px-4 py-2.5">
            <p className="min-w-0 truncate text-[11px] font-medium text-foreground">{s.subject_name}</p>
            <Badge status={s.status} />
          </div>
        ))}
        {data.sessions.length > 2 && (
          <p className="px-4 py-1.5 text-[10px] text-muted-foreground">+{data.sessions.length - 2} buổi khác...</p>
        )}
      </div>
...
function DetailDialog({ open, dateKey, data, onClose }: {
  open: boolean; dateKey: string | null; data: DayData | null; onClose: () => void;
}) {
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md rounded-2xl border-border bg-card p-0 overflow-hidden">
        <DialogHeader className="border-b border-border px-5 py-4">
          ...
          <p className="text-xs text-muted-foreground">Chi tiết điểm danh từng buổi học</p>
        </DialogHeader>
            ...
            <div key={i} className="overflow-hidden rounded-xl border border-border bg-background">
              ...
              <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-border/60">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-foreground leading-tight">{s.subject_name}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">{s.class_code} · Buổi số {s.session_no}</p>
                </div>
                <Badge status={s.status} />
              </div>
              ...
              <div className="px-4 py-2.5 space-y-1.5">
                <p className="text-[11px] text-muted-foreground">
                  <span className="font-medium text-foreground">Giáo viên:</span> {s.teacher_name}
                </p>
                ...
                {s.status === 'ABSENT' && (
                  <div className="rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-[11px] text-red-700">
                    ⚠️ Buổi vắng này ảnh hưởng trực tiếp đến tỷ lệ chuyên cần. Liên hệ nhà trường nếu có lý do.
                  </div>
                )}
                {s.status === 'LATE' && (
                  <div className="rounded-lg border border-amber-100 bg-amber-50 px-3 py-2 text-[11px] text-amber-700">
                    ⏰ Đi muộn — nên thông báo cho giáo viên để được ghi nhận lý do.
                  </div>
                )}
              </div>
            </div>
...
export function ParentAttendanceCalendarCard({
  ...
  const cells = useMemo(() => {
    ...
    for (let i = 0; i < firstDay; i++) result.push({ day: null, key: `pad-${i}` });
        ...
        <div className="p-5">
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={() => { setHoverKey(null); setHoverRect(null); setViewDate(new Date(today.getFullYear(), today.getMonth(), 1)); }}
            ...
            {cells.map(({ day, key }) => {
              ...
              return (
                <div
                  ...
                  onMouseEnter={data ? (e) => {
                    cancelHide();
                    showPopup(key, (e.currentTarget as HTMLElement).getBoundingClientRect());
                  } : undefined}
                  onMouseLeave={data ? scheduleHide : undefined}
                >
                  <span className={`text-[13px] font-bold leading-none ${isToday ? 'text-sky-600' : 'text-foreground'}`}>
                    {day}
                  </span>
                  {/* One dot per session */}
                  {data && data.sessions.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap justify-center gap-[3px]">
                      {data.sessions.slice(0, 4).map((sess, si) => (
                        <span key={si} className={`h-1.5 w-1.5 rounded-full ${S[sess.status].dot}`} />
                      ))}
                    </div>
                  )}
                </div>
          ...
          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1.5 border-t border-border pt-4 text-[11px] text-muted-foreground">
            {(['PRESENT','LATE','ABSENT'] as AttendanceRecordStatus[]).map(s => (
